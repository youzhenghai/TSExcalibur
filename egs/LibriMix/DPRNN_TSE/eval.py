...
def main(conf):
    ...
    if not conf["from_checkpoint"]:
        model = DPRNNSpeTasNet.from_pretrained(conf["model_path"])
    else:
        print("error this is prepara")
        model = DPRNNSpeTasNet(**conf["train_conf"]["filterbank"],
    ...
    for idx in tqdm(range(len(test_set))):
        ...
        mix, source, enroll, enroll_len, _ = test_set[idx]
        enroll_len = torch.tensor(enroll_len)
        mix, source, enroll, enroll_len = tensors_to_device([mix, source, enroll, enroll_len], device=model_device)
        # print("mix.shape: ", mix.shape)
        # print("source.shape: ", source.shape)
        # print("enroll.shape: ", enroll.shape)


        est_source,_ = model(mix.unsqueeze(0), enroll.unsqueeze(0), enroll_len)
